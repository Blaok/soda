...
def print_wrapped(p, stencil):
    ...
    p.println('bool %s = halide_rewrite_buffer(var_%s_buffer, %d%s%s%s%s);' % (p.new_var(), stencil.output.name, TYPE_WIDTH[stencil.output.type]/8, output_str[0], output_str[1], output_str[2], output_str[3]))
    ...
    input_size = ['0']*4
    for dim in range(stencil.dim):
        p.println('int32_t %s = %s_size_dim_%d + %d;' % (p.new_var(), stencil.output.name, dim, get_stencil_dim(get_overall_stencil_window(stencil.input, stencil.output))[dim]-1))
    ...
    p.println('bool %s = halide_rewrite_buffer(var_%s_buffer, %d%s%s%s%s);' % (p.new_var(), stencil.input.name, TYPE_WIDTH[stencil.input.type]/8, input_str[0], input_str[1], input_str[2], input_str[3]))
    p.println('(void)%s;' % p.last_var())
    ...
    p.println('int dram_bank = %d;' % MAX_DRAM_BANK)
    ...
    p.println('if(%s)' % ' || '.join(['(dram_bank>%d && (%s || %s))' % (x, ' || '.join(['!var_%s_%d_bank_%d_cl' % (stencil.input.name, c, x) for c in range(stencil.input.chan)]), ' || '.join(['!var_%s_%d_bank_%d_cl' % (stencil.output.name, c, x) for c in range(stencil.output.chan)])) for x in range(MAX_DRAM_BANK)]+['!var_%s_cl' % param.name for param in stencil.extra_params.values()]))
    p.do_scope()
    ...
    p.println('cl_event write_events[%d];' % (MAX_DRAM_BANK*stencil.input.chan+len(stencil.extra_params)))
    ...
    p.println('for(int32_t %c = 0; %c < %s_size_dim_%d; ++%c)' % (COORDS_IN_TILE[stencil.dim-1], COORDS_IN_TILE[stencil.dim-1], stencil.input.name, stencil.dim-1, COORDS_IN_TILE[stencil.dim-1]))
    p.do_scope()
    for dim in range(stencil.dim-2, -1, -1):
        p.println('for(int32_t %c = 0; %c < actual_tile_size_dim_%d; ++%c)' % (COORDS_IN_TILE[dim], COORDS_IN_TILE[dim], dim, COORDS_IN_TILE[dim]))
        p.do_scope()

    p.println('// (%s) is coordinates in tiled image' % ', '.join(COORDS_TILED))
    p.println('// (%s) is coordinates in original image' % ', '.join(COORDS_IN_ORIG))
    p.println('// (%s) is coordinates in a tile' % ', '.join(COORDS_IN_TILE))
    ...
    for dim in range(stencil.dim-1):
        p.println('int32_t %c = tile_index_dim_%d*(TILE_SIZE_DIM_%d-STENCIL_DIM_%d+1)+%c;' % (COORDS_IN_ORIG[dim], dim, dim, dim, COORDS_IN_TILE[dim]))
    p.println('int32_t %c = %c;' % (COORDS_IN_ORIG[stencil.dim-1], COORDS_IN_TILE[stencil.dim-1]))
    ...
    p.println('int64_t original_offset = %s;' % '+'.join(['%c*var_%s_stride_%d' % (COORDS_IN_ORIG[x], stencil.input.name, x) for x in range(stencil.dim)]))
    ...
    p.println('cl_event read_events[%d];' % (MAX_DRAM_BANK*stencil.output.chan))
    for i in range(MAX_DRAM_BANK):
        for c in range(stencil.output.chan):
    ...
    if stencil.preserve_border:
        overall_stencil_window = get_overall_stencil_window(stencil.output.parent.preserve_border_from(), stencil.output)
        overall_stencil_offset = get_stencil_window_offset(overall_stencil_window)
        overall_stencil_dim = get_stencil_dim(overall_stencil_window)
        p.println('for(int32_t %c = 0; %c < %s_size_dim_%d; ++%c)' % (COORDS_IN_TILE[stencil.dim-1], COORDS_IN_TILE[stencil.dim-1], stencil.input.name, stencil.dim-1, COORDS_IN_TILE[stencil.dim-1]))
        p.do_scope()
        for dim in range(stencil.dim-2, -1, -1):
            p.println('for(int32_t %c = tile_index_dim_%d==0 ? 0 : %d; %c < actual_tile_size_dim_%d-(tile_index_dim_%d==tile_num_dim_%d-1 ? 0 : %d); ++%c)' % (COORDS_IN_TILE[dim], dim, overall_stencil_offset[dim], COORDS_IN_TILE[dim], dim, dim, dim, overall_stencil_dim[dim]-1-overall_stencil_offset[dim], COORDS_IN_TILE[dim]))
            p.do_scope()
    else:
        overall_stencil_window = get_overall_stencil_window(stencil.input, stencil.output)
        overall_stencil_offset = get_stencil_window_offset(overall_stencil_window)
        overall_stencil_dim = get_stencil_dim(overall_stencil_window)
        p.println('for(int32_t %c = %d; %c < %s_size_dim_%d-%d; ++%c)' % (COORDS_IN_TILE[stencil.dim-1], overall_stencil_offset[stencil.dim-1], COORDS_IN_TILE[stencil.dim-1], stencil.output.name, stencil.dim-1, overall_stencil_dim[stencil.dim-1]-1-overall_stencil_offset[stencil.dim-1], COORDS_IN_TILE[stencil.dim-1]))
        p.do_scope()
        for dim in range(stencil.dim-2, -1, -1):
            p.println('for(int32_t %c = %d; %c < actual_tile_size_dim_%d-%d; ++%c)' % (COORDS_IN_TILE[dim], overall_stencil_offset[dim], COORDS_IN_TILE[dim], dim, overall_stencil_dim[dim]-1-overall_stencil_offset[dim], COORDS_IN_TILE[dim]))
            p.do_scope()
    ...
    p.println('int64_t original_offset = %s;' % '+'.join(['%c*var_%s_stride_%d' % (COORDS_IN_ORIG[x], stencil.output.name, x) for x in range(stencil.dim)]))
    p.println('switch(tiled_offset%dram_bank)')
    p.do_scope()
    for i in range(MAX_DRAM_BANK):
        p.println('case %d:' % i)
...
def print_check_elem_size(p, buffer_name, buffer_type):
    p.println('bool %s = var_%s_elem_size == %d;' % (p.new_var(), buffer_name, TYPE_WIDTH[buffer_type]/8))
    p.println('if(!%s)' % p.last_var())
    p.do_scope()
    p.println('int32_t %s = halide_error_bad_elem_size(nullptr, "Buffer %s", "%s", var_%s_elem_size, %d);' % (p.new_var(), buffer_name, buffer_type, buffer_name, TYPE_WIDTH[buffer_type]/8))
...
def print_test(p, stencil):
    ...
    if stencil.input.chan>1:
        p.println('for(int32_t %c = 0; %c<%d; ++%c)' % (COORDS_IN_ORIG[stencil.dim], COORDS_IN_ORIG[stencil.dim], stencil.input.chan, COORDS_IN_ORIG[stencil.dim]))
        p.do_scope()
    for d in range(0, stencil.dim):
        dim = stencil.dim-d-1
        p.println('for(int32_t %c = 0; %c<dims[%d]; ++%c)' % (COORDS_IN_ORIG[dim], COORDS_IN_ORIG[dim], dim, COORDS_IN_ORIG[dim]))
        p.do_scope()
    init_val = '+'.join(COORDS_IN_ORIG[0:input_dim])
    if is_float(stencil.input.type):
    ...
    p.println('%s_img[%s] = %s;' % (stencil.input.name, '+'.join('%c*%s.stride[%d]' % (COORDS_IN_ORIG[d], stencil.input.name, d) for d in range(input_dim)), init_val))
    ...
    for param in stencil.extra_params.values():
        ...
        for dim, size in enumerate(param.size):
            p.println('for(int32_t %c = 0; %c<%d; ++%c)' % (COORDS_IN_ORIG[dim], COORDS_IN_ORIG[dim], size, COORDS_IN_ORIG[dim]))
            p.do_scope()
        p.println('%s_img%s = %s;' % (param.name, reduce(operator.add, ['[%c]' % (COORDS_IN_ORIG[d]) for d in range(len(param.size))]), '+'.join(COORDS_IN_ORIG[0:len(param.size)])))
        for d in param.size:
            p.un_scope()
        p.println()
    ...
    for s in stencil.get_stages_chronologically():
        ...
        stencil_window = get_overall_stencil_window(stencil_window_input, s.output)
        stencil_dim = get_stencil_dim(stencil_window)
        output_idx = get_stencil_window_offset(stencil_window)
        for d in range(0, stencil.dim):
            dim = stencil.dim-d-1
            p.println('for(int32_t %c = %d; %c<dims[%d]-%d; ++%c)' % (COORDS_IN_ORIG[dim], output_idx[dim], COORDS_IN_ORIG[dim], dim, stencil_dim[dim]-output_idx[dim]-1, COORDS_IN_ORIG[dim]))
            p.do_scope()
        for e in s.expr:
        ...
        if len(s.output.children)==0:
            for c in range(stencil.output.chan):
                ...
                if is_float(stencil.output.type):
        ...
        if s.preserve_border_from():
            ...
            stencil_window = get_overall_stencil_window(bb, s.output)
            stencil_dim = get_stencil_dim(stencil_window)
            output_idx = get_stencil_window_offset(stencil_window)
            for d in range(0, stencil.dim):
                dim = stencil.dim-d-1
                p.println('for(int32_t %c = 0; %c<dims[%d]; ++%c)' % (COORDS_IN_ORIG[dim], COORDS_IN_ORIG[dim], dim, COORDS_IN_ORIG[dim]))
                p.do_scope()
            p.println('if(!(%s))' % ' && '.join('%c>=%d && %c<dims[%d]-%d' % (COORDS_IN_ORIG[d], output_idx[d], COORDS_IN_ORIG[d], d, stencil_dim[d]-output_idx[d]-1) for d in range(stencil.dim)))
            p.do_scope()
...
def print_code(stencil, host_file):
    ...
    print_define(p, 'BURST_WIDTH', stencil.burst_width)
    print_define(p, 'PIXEL_WIDTH_I', TYPE_WIDTH[stencil.input.type])
    print_define(p, 'PIXEL_WIDTH_O', TYPE_WIDTH[stencil.output.type])

    if stencil.preserve_border:
        overall_stencil_window = get_overall_stencil_window(
            stencil.output.parent.preserve_border_from(), stencil.output)
    else:
        overall_stencil_window = get_overall_stencil_window(
            stencil.input, stencil.output)

    overall_stencil_distance = get_stencil_distance(overall_stencil_window,
        stencil.tile_size)

    for i, dim in enumerate(get_stencil_dim(overall_stencil_window)):
        print_define(p, 'STENCIL_DIM_%d' % i, dim)
    stencil_offset = overall_stencil_distance - serialize(
        get_stencil_window_offset(overall_stencil_window), stencil.tile_size)
    if stencil.preserve_border:
        stencil_offset *= stencil.iterate

    overall_stencil_distance = max(overall_stencil_distance, stencil_offset)

    print_define(p, 'STENCIL_OFFSET', stencil_offset)
    print_define(p, 'STENCIL_DISTANCE', overall_stencil_distance)
    p.println()
