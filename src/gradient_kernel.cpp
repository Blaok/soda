...
void compute(bool compute_flag, float output[TILE_SIZE_DIM0*TILE_SIZE_DIM1], float input[TILE_SIZE_DIM0*TILE_SIZE_DIM1], int32_t tile_index, int32_t tile_num_dim0, int32_t var_f_extent_0, int32_t var_f_extent_1, int32_t var_f_min_0, int32_t var_f_min_1)
{
    if(compute_flag)
    {
        ...
        float stencil_buf[STENCIL_DISTANCE+UNROLL_FACTOR];
...
#pragma HLS unroll
                if(input_index >= STENCIL_DISTANCE/UNROLL_FACTOR)
                {
                        ...
                        output_index < TILE_SIZE_DIM0*TILE_SIZE_DIM1)
                    {
                        ...
                        float input_c = stencil_buf[unroll_index+TILE_SIZE_DIM0*1+1];
                        float input_u = stencil_buf[unroll_index+TILE_SIZE_DIM0*0+1];
                        float input_d = stencil_buf[unroll_index+TILE_SIZE_DIM0*2+1];
                        float input_l = stencil_buf[unroll_index+TILE_SIZE_DIM0*1+0];
                        float input_r = stencil_buf[unroll_index+TILE_SIZE_DIM0*1+2];
                        if(p >= var_f_min_0 &&
                           q >= var_f_min_1 &&
                           ...
                           q < var_f_min_1 + var_f_extent_1)
                        {
                            ...
                            output[output_index] = assign_122;
                        }
                    }
