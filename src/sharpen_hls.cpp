...
#define STENCIL_DISTANCE ((TILE_SIZE_DIM0)*2)
...
void compute(bool compute_flag, uint16_t output[TILE_SIZE_DIM0*TILE_SIZE_DIM1], uint16_t input[TILE_SIZE_DIM0*TILE_SIZE_DIM1], int32_t tile_index, int32_t tile_num_dim0, int32_t var_sharpen_y_extent_0, int32_t var_sharpen_y_extent_1, int32_t var_sharpen_y_min_0, int32_t var_sharpen_y_min_1)
{
    if(compute_flag)
    {
...
#pragma HLS unroll factor=UNROLL_FACTOR
                ...
                uint16_t input_1 = input[TILE_SIZE_DIM0*(0+j)+i+1];
                uint16_t input_3 = input[TILE_SIZE_DIM0*(1+j)+i+0];
                uint16_t input_4 = input[TILE_SIZE_DIM0*(1+j)+i+1];
                uint16_t input_5 = input[TILE_SIZE_DIM0*(1+j)+i+2];
                uint16_t input_7 = input[TILE_SIZE_DIM0*(2+j)+i+1];
                if(p >= var_sharpen_y_min_0 &&
                   q >= var_sharpen_y_min_1 &&
                   p < var_sharpen_y_min_0 + var_sharpen_y_extent_0 &&
                   q < var_sharpen_y_min_1 + var_sharpen_y_extent_1)
                {
                    uint16_t assign_95 = input_4;
                    uint16_t assign_96 = (uint16_t)(5);                       
                    uint16_t assign_97 = assign_95 * assign_96;               
                    uint16_t assign_102 = input_1;
                    uint16_t assign_103 = assign_97 - assign_102;             
                    uint16_t assign_109 = input_3;
                    uint16_t assign_110 = assign_103 - assign_109;            
                    uint16_t assign_111 = input_5; 
                    uint16_t assign_112 = assign_110 - assign_111;            
                    uint16_t assign_114 = input_7;
                    uint16_t assign_115 = assign_112 - assign_114;            
                    output[TILE_SIZE_DIM0*j+i] = assign_115;
...
void sharpen_kernel(ap_uint<BURST_WIDTH>* var_sharpen_y, ap_uint<BURST_WIDTH>* var_p0, int32_t tile_num_dim0, int32_t tile_num_dim1, int32_t var_sharpen_y_extent_0, int32_t var_sharpen_y_extent_1, int32_t var_sharpen_y_min_0, int32_t var_sharpen_y_min_1)
{
#pragma HLS INTERFACE m_axi port=var_sharpen_y offset=slave depth=65536 bundle=gmem1 latency=120
#pragma HLS INTERFACE m_axi port=var_p0 offset=slave depth=65536 bundle=gmem2 latency=120

#pragma HLS INTERFACE s_axilite port=var_sharpen_y bundle=control
...
#pragma HLS INTERFACE s_axilite port=var_sharpen_y_extent_0 bundle=control
#pragma HLS INTERFACE s_axilite port=var_sharpen_y_extent_1 bundle=control
#pragma HLS INTERFACE s_axilite port=var_sharpen_y_min_0 bundle=control
#pragma HLS INTERFACE s_axilite port=var_sharpen_y_min_1 bundle=control
...
#pragma HLS array_partition variable=output_1 cyclic factor=UNROLL_FACTOR dim=1
    ...
    for (tile_index = 0; tile_index < total_tile_num+2; ++tile_index)
    {
        ...
        if(tile_index%2==0)
        {
            ...
            compute(compute_flag, output_1, input_1, tile_index-1, tile_num_dim0, var_sharpen_y_extent_0, var_sharpen_y_extent_1, var_sharpen_y_min_0, var_sharpen_y_min_1);
            store(store_flag, var_sharpen_y, output_0, tile_index-2);
        }
        else
        {
            load(load_flag, input_1, var_p0, tile_index);
            compute(compute_flag, output_0, input_0, tile_index-1, tile_num_dim0, var_sharpen_y_extent_0, var_sharpen_y_extent_1, var_sharpen_y_min_0, var_sharpen_y_min_1);
            store(store_flag, var_sharpen_y, output_1, tile_index-2);
        }
    }
}
