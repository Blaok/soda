...
static int blur_wrapped(buffer_t *var_input_buffer, buffer_t *var_output_buffer, const char* xclbin) HALIDE_FUNCTION_ATTRS {
    ...
    if (assign_5)
    {
        ...
        int64_t tile_burst_num = (tile_pixel_num-1)/BURST_LENGTH/2+1;
        int64_t tile_size_linearized_i = (CHANNEL_NUM_I == 1 ? tile_pixel_num : tile_burst_num*BURST_LENGTH*2*CHANNEL_NUM_I);
        int64_t tile_size_linearized_o = (CHANNEL_NUM_O == 1 ? tile_pixel_num : tile_burst_num*BURST_LENGTH*2*CHANNEL_NUM_O);
        int64_t extra_space_i = (CHANNEL_NUM_I == 1 ? tile_burst_num*BURST_LENGTH*2-tile_pixel_num : 0);
        int64_t extra_space_o = (CHANNEL_NUM_O == 1 ? tile_burst_num*BURST_LENGTH*2-tile_pixel_num : 0);

        uint16_t* var_input_buf_channel_1 = new uint16_t[tile_num_dim_0*tile_size_linearized_i/2];
        uint16_t* var_input_buf_channel_2 = new uint16_t[tile_num_dim_0*tile_size_linearized_i/2];
        uint16_t* var_output_buf_channel_1 = new uint16_t[tile_num_dim_0*tile_size_linearized_o/2];
        uint16_t* var_output_buf_channel_2 = new uint16_t[tile_num_dim_0*tile_size_linearized_o/2];
            ...
            for(int32_t c = 0; c < CHANNEL_NUM_I; ++c)
            {
                for(int32_t j = 0; j < input_size_dim_1; ++j)
                {
                    for(int32_t i = 0; i < actual_tile_size_dim_0; ++i)
                    {
                        ...
                        int32_t burst_index = (j*TILE_SIZE_DIM_0+i)/(BURST_LENGTH*2);
                        int32_t burst_residue = (j*TILE_SIZE_DIM_0+i)%(BURST_LENGTH*2);
                        int32_t p = tile_index_dim_0*(TILE_SIZE_DIM_0-STENCIL_DIM_0+1)+i;
                        int32_t q = j;
                        int64_t tiled_offset = (tile_index_dim_0*tile_pixel_num+burst_index*BURST_LENGTH*2)*CHANNEL_NUM_I+c*BURST_LENGTH*2+burst_residue;
                        int64_t original_offset = (q*var_input_stride_1+p)*CHANNEL_NUM_I+c;
                        if(tiled_offset%2==0)
                        {
                            var_input_buf_channel_1[tiled_offset/2] = var_input[original_offset];
                        }
                        else
                        {
                            var_input_buf_channel_2[tiled_offset/2] = var_input[original_offset];
                        }
                    }
                }
            }
...
#define TARGET_DEVICE GET_STRING(SDA_PLATFORM)
        ...
        cl_mem var_input_channel_1_cl;                   // device memory used for the input array
        cl_mem var_input_channel_2_cl;                   // device memory used for the input array
        cl_mem var_output_channel_1_cl;               // device memory used for the output array
        cl_mem var_output_channel_2_cl;               // device memory used for the output array
        ...
        cl_mem_ext_ptr_t input_ext_channel_1, output_ext_channel_1;
        cl_mem_ext_ptr_t input_ext_channel_2, output_ext_channel_2;
        output_ext_channel_1.flags = XCL_MEM_DDR_BANK0;
        output_ext_channel_2.flags = XCL_MEM_DDR_BANK1;
        input_ext_channel_1.flags = XCL_MEM_DDR_BANK2;
        input_ext_channel_2.flags = XCL_MEM_DDR_BANK3;
        input_ext_channel_1.obj = 0;
        input_ext_channel_1.param = 0;
        input_ext_channel_2.obj = 0;
        input_ext_channel_2.param = 0;
        output_ext_channel_1.obj = 0;
        output_ext_channel_1.param = 0;
        output_ext_channel_2.obj = 0;
        output_ext_channel_2.param = 0;

        var_input_channel_1_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint16_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i)/2, & input_ext_channel_1, NULL);
        var_input_channel_2_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint16_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i)/2, & input_ext_channel_2, NULL);
        var_output_channel_1_cl = clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint16_t) * (tile_num_dim_0*tile_size_linearized_o+extra_space_o)/2, &output_ext_channel_1, NULL);
        var_output_channel_2_cl = clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint16_t) * (tile_num_dim_0*tile_size_linearized_o+extra_space_o)/2, &output_ext_channel_2, NULL);
        if (!var_input_channel_1_cl || !var_output_channel_1_cl || !var_input_channel_2_cl || !var_output_channel_2_cl)
        {
        ...
        err = clEnqueueWriteBuffer(commands, var_input_channel_1_cl, CL_FALSE, 0, sizeof(uint16_t) * tile_num_dim_0*tile_size_linearized_i/2, var_input_buf_channel_1, 0, NULL, &writeevent);
        err = clEnqueueWriteBuffer(commands, var_input_channel_2_cl, CL_FALSE, 0, sizeof(uint16_t) * tile_num_dim_0*tile_size_linearized_i/2, var_input_buf_channel_2, 0, NULL, &writeevent);
        if (err != CL_SUCCESS)
        {
        ...
        int64_t extra_space_i_coalesed = extra_space_i/(BURST_WIDTH/PIXEL_WIDTH_I)/2;
        int64_t extra_space_o_coalesed = extra_space_o/(BURST_WIDTH/PIXEL_WIDTH_O)/2;
        int32_t total_burst_num = tile_num_dim_0*tile_burst_num;

        int kernel_arg = 0;
        err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_output_channel_1_cl);
        err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_output_channel_2_cl);
        err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_input_channel_1_cl);
        err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_input_channel_2_cl);
        ...
        err = clEnqueueReadBuffer(commands, var_output_channel_1_cl, CL_FALSE, 0, sizeof(uint16_t) * tile_num_dim_0*tile_size_linearized_o/2, var_output_buf_channel_1, 0, NULL, &readevent );
        err = clEnqueueReadBuffer(commands, var_output_channel_2_cl, CL_FALSE, 0, sizeof(uint16_t) * tile_num_dim_0*tile_size_linearized_o/2, var_output_buf_channel_2, 0, NULL, &readevent );
        if (err != CL_SUCCESS)
        {
        ...
        clReleaseMemObject(var_input_channel_1_cl);
        clReleaseMemObject(var_input_channel_2_cl);
        clReleaseMemObject(var_output_channel_1_cl);
        clReleaseMemObject(var_output_channel_2_cl);
            ...
            for(int32_t c = 0; c < CHANNEL_NUM_O; ++c)
            {
                for(int32_t j = 0; j < input_size_dim_1-STENCIL_DIM_1+1; ++j)
                {
                    for(int32_t i = 0; i < actual_tile_size_dim_0-STENCIL_DIM_0+1; ++i)
                    {
                        ...
                        // (p, q, r, s) is coordinates in original image
                        // (i, j, k, l) is coordinates in a tile
                        int32_t burst_index = (j*TILE_SIZE_DIM_0+i+STENCIL_DISTANCE)/(BURST_LENGTH*2);
                        int32_t burst_residue = (j*TILE_SIZE_DIM_0+i+STENCIL_DISTANCE)%(BURST_LENGTH*2);
                        int32_t p = tile_index_dim_0*(TILE_SIZE_DIM_0-STENCIL_DIM_0+1)+i;
                        int32_t q = j;
                        int64_t tiled_offset = (tile_index_dim_0*tile_pixel_num+burst_index*BURST_LENGTH*2)*CHANNEL_NUM_O+c*BURST_LENGTH*2+burst_residue;
                        ...
                        if(tiled_offset%2==0)
                        {
                            var_output[original_offset] = var_output_buf_channel_1[tiled_offset/2];
                        }
                        else
                        {
                            var_output[original_offset] = var_output_buf_channel_2[tiled_offset/2];
                        }
                    }
                }
            }
        }
        delete[] var_output_buf_channel_1;
        delete[] var_output_buf_channel_2;
        delete[] var_input_buf_channel_1;
        delete[] var_input_buf_channel_2;
    } // if assign_5
    return 0;
}
