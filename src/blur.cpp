...
int load_xclbin2_to_memory(const char *filename, char **kernel_binary, char** device)
{
    uint64_t size = 0;
...
static int blur_wrapped(buffer_t *var_input_buffer, buffer_t *var_output_buffer, const char* xclbin) HALIDE_FUNCTION_ATTRS
{
    ...
    if(assign_5)
    {
        ...
        cl_mem var_input_chan_0_cl;
        cl_mem var_input_chan_1_cl;
        cl_mem var_input_chan_2_cl;
        cl_mem var_input_chan_3_cl;
        cl_mem var_output_chan_0_cl;
        cl_mem var_output_chan_1_cl;
        cl_mem var_output_chan_2_cl;
        cl_mem var_output_chan_3_cl;

        uint64_t var_input_buf_size = sizeof(uint16_t)*tile_num_dim_0*tile_size_linearized_i/dram_chan;
        uint64_t var_output_buf_size = sizeof(uint16_t)*tile_num_dim_0*tile_size_linearized_o/dram_chan;

        unsigned char *kernel_binary;
        const char *device_name;
        char target_device_name[64];
        fprintf(*error_report, "INFO: Loading %s\n", xclbin);
        int kernel_binary_size = load_xclbin2_to_memory(xclbin, (char **) &kernel_binary, (char**)&device_name);
        ...
        int status;
        size_t kernel_binary_sizes[1] = {kernel_binary_size};
        ...
        var_input_chan_0_cl = dram_chan > 0 ? clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, var_input_buf_size, &input_ext_chan_0, nullptr) : nullptr;
        var_input_chan_1_cl = dram_chan > 1 ? clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, var_input_buf_size, &input_ext_chan_1, nullptr) : nullptr;
        var_input_chan_2_cl = dram_chan > 2 ? clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, var_input_buf_size, &input_ext_chan_2, nullptr) : nullptr;
        var_input_chan_3_cl = dram_chan > 3 ? clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, var_input_buf_size, &input_ext_chan_3, nullptr) : nullptr;
        var_output_chan_0_cl = dram_chan > 0 ? clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, var_output_buf_size, &output_ext_chan_0, nullptr) : nullptr;
        var_output_chan_1_cl = dram_chan > 1 ? clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, var_output_buf_size, &output_ext_chan_1, nullptr) : nullptr;
        var_output_chan_2_cl = dram_chan > 2 ? clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, var_output_buf_size, &output_ext_chan_2, nullptr) : nullptr;
        var_output_chan_3_cl = dram_chan > 3 ? clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, var_output_buf_size, &output_ext_chan_3, nullptr) : nullptr;
        if((dram_chan>0 && (!var_input_chan_0_cl || !var_output_chan_0_cl)) || (dram_chan>1 && (!var_input_chan_1_cl || !var_output_chan_1_cl)) || (dram_chan>2 && (!var_input_chan_2_cl || !var_output_chan_2_cl)) || (dram_chan>3 && (!var_input_chan_3_cl || !var_output_chan_3_cl)))
        {
        ...
        uint16_t* var_input_buf_chan_0 = dram_chan>0 ? (uint16_t*)clEnqueueMapBuffer(commands, var_input_chan_0_cl, CL_FALSE, CL_MAP_WRITE, 0, var_input_buf_size, 0, nullptr, write_events+0, &err) : nullptr;
        uint16_t* var_input_buf_chan_1 = dram_chan>1 ? (uint16_t*)clEnqueueMapBuffer(commands, var_input_chan_1_cl, CL_FALSE, CL_MAP_WRITE, 0, var_input_buf_size, 0, nullptr, write_events+1, &err) : nullptr;
        uint16_t* var_input_buf_chan_2 = dram_chan>2 ? (uint16_t*)clEnqueueMapBuffer(commands, var_input_chan_2_cl, CL_FALSE, CL_MAP_WRITE, 0, var_input_buf_size, 0, nullptr, write_events+2, &err) : nullptr;
        uint16_t* var_input_buf_chan_3 = dram_chan>3 ? (uint16_t*)clEnqueueMapBuffer(commands, var_input_chan_3_cl, CL_FALSE, CL_MAP_WRITE, 0, var_input_buf_size, 0, nullptr, write_events+3, &err) : nullptr;
        clWaitForEvents(dram_chan, write_events);
            ...
            for(int32_t c = 0; c < CHANNEL_NUM_I; ++c)
            {
                for(int32_t j = 0; j < input_size_dim_1; ++j)
                {
                    for(int32_t i = 0; i < actual_tile_size_dim_0; ++i)
                    {
                        ...
                        int32_t burst_index = (i+j*TILE_SIZE_DIM_0)/(BURST_WIDTH/PIXEL_WIDTH_I*dram_chan);
                        int32_t burst_residue = (i+j*TILE_SIZE_DIM_0)%(BURST_WIDTH/PIXEL_WIDTH_I*dram_chan);
                        ...
                        int64_t original_offset = (p*var_input_stride_0+q*var_input_stride_1)*CHANNEL_NUM_I+c;
        ...
        err = 0;
        err |= dram_chan>0 ? clEnqueueUnmapMemObject(commands, var_input_chan_0_cl, var_input_buf_chan_0, 0, nullptr, write_events+0) : err;
        err |= dram_chan>1 ? clEnqueueUnmapMemObject(commands, var_input_chan_1_cl, var_input_buf_chan_1, 0, nullptr, write_events+1) : err;
        err |= dram_chan>2 ? clEnqueueUnmapMemObject(commands, var_input_chan_2_cl, var_input_buf_chan_2, 0, nullptr, write_events+2) : err;
        err |= dram_chan>3 ? clEnqueueUnmapMemObject(commands, var_input_chan_3_cl, var_input_buf_chan_3, 0, nullptr, write_events+3) : err;
        ...
        err = 0;
        fprintf(*error_report, "INFO: Using %d DRAM channel%s%s\n", dram_chan, dram_chan>1 ? "s" : "", dram_separate ? ", separated" : "");
        ...
        fprintf(*error_report, "INFO: output_extent_0 = %d, output_extent_1 = %d\n", output_size_dim_0, output_size_dim_1);
        fprintf(*error_report, "INFO: output_min_0 = %d, output_min_1 = %d\n", var_output_min_0, var_output_min_1);
        ...
        if(dram_chan>0) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_output_chan_0_cl);
        if(dram_chan>1) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_output_chan_1_cl);
        if(dram_chan>2) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_output_chan_2_cl);
        if(dram_chan>3) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_output_chan_3_cl);
        if(dram_chan>0) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_input_chan_0_cl);
        if(dram_chan>1) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_input_chan_1_cl);
        if(dram_chan>2) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_input_chan_2_cl);
        if(dram_chan>3) err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_input_chan_3_cl);
        ...
        uint16_t* var_output_buf_chan_0 = dram_chan>0 ? (uint16_t*)clEnqueueMapBuffer(commands, var_output_chan_0_cl, CL_FALSE, CL_MAP_READ, 0, var_output_buf_size, 0, nullptr, read_events+0, &err) : nullptr;
        uint16_t* var_output_buf_chan_1 = dram_chan>1 ? (uint16_t*)clEnqueueMapBuffer(commands, var_output_chan_1_cl, CL_FALSE, CL_MAP_READ, 0, var_output_buf_size, 0, nullptr, read_events+1, &err) : nullptr;
        uint16_t* var_output_buf_chan_2 = dram_chan>2 ? (uint16_t*)clEnqueueMapBuffer(commands, var_output_chan_2_cl, CL_FALSE, CL_MAP_READ, 0, var_output_buf_size, 0, nullptr, read_events+2, &err) : nullptr;
        uint16_t* var_output_buf_chan_3 = dram_chan>3 ? (uint16_t*)clEnqueueMapBuffer(commands, var_output_chan_3_cl, CL_FALSE, CL_MAP_READ, 0, var_output_buf_size, 0, nullptr, read_events+3, &err) : nullptr;
        clWaitForEvents(dram_chan, read_events);
        ...
        if(dram_chan>0)
        {
            clEnqueueUnmapMemObject(commands, var_output_chan_0_cl, var_output_buf_chan_0, 0, nullptr, read_events+0);
        ...
        if(dram_chan>1)
        {
            clEnqueueUnmapMemObject(commands, var_output_chan_1_cl, var_output_buf_chan_1, 0, nullptr, read_events+1);
        ...
        if(dram_chan>2)
        {
            clEnqueueUnmapMemObject(commands, var_output_chan_2_cl, var_output_buf_chan_2, 0, nullptr, read_events+2);
            clReleaseMemObject(var_input_chan_2_cl);
            clReleaseMemObject(var_output_chan_2_cl);
        }

        if(dram_chan>3)
        {
            clEnqueueUnmapMemObject(commands, var_output_chan_3_cl, var_output_buf_chan_3, 0, nullptr, read_events+3);
            clReleaseMemObject(var_input_chan_3_cl);
            clReleaseMemObject(var_output_chan_3_cl);
        }
        clWaitForEvents(dram_chan, read_events);
