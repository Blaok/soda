...
def accumulate_fifo(estimation_routed, width, depth):
    ...
    if fifo_size > 1024:
        estimation_routed['BRAM'] += math.ceil(fifo_size/32768)
    else:
...
def print_estimation(stencil, model_file, output_file):
        ...
        if isinstance(node, ForwardNode):
            ...
            if forward_key not in model:
                msg = 'forward<%s> not found in model %s' % (node.tensor.type,
                                                             model_file.name)
                raise SemanticError(msg)
            accumulate_resources(estimation_hls, model[forward_key]['base'])
            accumulate_resources(estimation_hls,
    ...
    estimation_routed['BRAM'] = estimation_hls['BRAM_18K']/2
    estimation_routed['REG'] = estimation_hls['FF']*model['ff_coeff']
    estimation_routed['LUT'] = estimation_hls['LUT']*model['lut_coeff']
