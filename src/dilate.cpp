...
static int dilate_wrapped(buffer_t *var_input_buffer, buffer_t *var_output_buffer, const char* xclbin) HALIDE_FUNCTION_ATTRS {
    ...
    uint8_t *var_output = (uint8_t *)(var_output_buffer->host);
    (void)var_output;
    ...
    if (assign_5)
    {
        ...
        if (!assign_6)         {
            int32_t assign_7 = halide_error_bad_elem_size(NULL, "Output buffer output", "uint8", var_output_elem_size, 1);
            return assign_7;
        }
        bool assign_8 = var_input_elem_size == 1;
        if (!assign_8)         {
            int32_t assign_9 = halide_error_bad_elem_size(NULL, "Input buffer input", "uint8", var_input_elem_size, 1);
            return assign_9;
        }
        ...
        uint8_t* var_input_buf_chan_0 = new uint8_t[tile_num_dim_0*tile_size_linearized_i/4];
        uint8_t* var_input_buf_chan_1 = new uint8_t[tile_num_dim_0*tile_size_linearized_i/4];
        uint8_t* var_input_buf_chan_2 = new uint8_t[tile_num_dim_0*tile_size_linearized_i/4];
        uint8_t* var_input_buf_chan_3 = new uint8_t[tile_num_dim_0*tile_size_linearized_i/4];
...
#define TARGET_DEVICE GET_STRING(SDA_PLATFORM)
        ...
        kernel = clCreateKernel(program, "dilate_kernel", &err);
        ...
        var_input_chan_0_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint8_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i)/4, & input_ext_chan_0, NULL);
        var_input_chan_1_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint8_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i)/4, & input_ext_chan_1, NULL);
        var_input_chan_2_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint8_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i)/4, & input_ext_chan_2, NULL);
        var_input_chan_3_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(uint8_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i)/4, & input_ext_chan_3, NULL);
...
int dilate(buffer_t *var_input_buffer, buffer_t *var_output_buffer, const char* xclbin) HALIDE_FUNCTION_ATTRS {
    uint8_t *var_input = (uint8_t *)(var_input_buffer->host);
    (void)var_input;
    const bool var_input_host_and_dev_are_null = (var_input_buffer->host == NULL) && (var_input_buffer->dev == 0);
    (void)var_input_host_and_dev_are_null;
    ...
    int32_t var_output_elem_size = var_output_buffer->elem_size;
    (void)var_output_elem_size;
    int32_t assign_81 = dilate_wrapped(var_input_buffer, var_output_buffer, xclbin);
