...
static int edge_wrapped(buffer_t *var_input_buffer, buffer_t *var_output_buffer, const char* xclbin) HALIDE_FUNCTION_ATTRS {
    ...
    if (assign_5)
    {
        ...
        uint8_t* var_input_buf = new uint8_t[tile_num_dim_0*tile_size_linearized_i];
...
#define TARGET_DEVICE GET_STRING(SDA_PLATFORM)
        ...
        var_input_cl  = clCreateBuffer(context,  CL_MEM_READ_ONLY, sizeof(uint8_t) * (tile_num_dim_0*tile_size_linearized_i+extra_space_i), NULL, NULL);
        ...
        err = clEnqueueWriteBuffer(commands, var_input_cl, CL_FALSE, 0, sizeof(uint8_t) * tile_num_dim_0*tile_size_linearized_i, var_input_buf, 0, NULL, &writeevent);
        if (err != CL_SUCCESS)
        {
