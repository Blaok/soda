...
INPUT_DEPTH=$(( ( (${DIM0}+${TILE_SIZE_DIM0}-23) / (${TILE_SIZE_DIM0}-22) ) * ( (${DIM1}+${TILE_SIZE_DIM1}-19) / (${TILE_SIZE_DIM1}-18) ) * ${TILE_SIZE_DIM0} * ${TILE_SIZE_DIM1} / 64 ))
OUTPUT_DEPTH=$(( ( (${DIM0}+${TILE_SIZE_DIM0}-23) / (${TILE_SIZE_DIM0}-22) ) * ( (${DIM1}+${TILE_SIZE_DIM1}-19) / (${TILE_SIZE_DIM1}-18) ) * ${TILE_SIZE_DIM0} * ${TILE_SIZE_DIM1} / 21 ))

sed -i 's/#pragma HLS INTERFACE m_axi port=var_processed offset=slave depth=.* bundle=gmem1 latency=120/#pragma HLS INTERFACE m_axi port=var_processed offset=slave depth='${OUTPUT_DEPTH}' bundle=gmem1 latency=120/' src/${KERNEL_SRCS}
sed -i 's/#pragma HLS INTERFACE m_axi port=var_input offset=slave depth=.* bundle=gmem2 latency=120/#pragma HLS INTERFACE m_axi port=var_input offset=slave depth='${INPUT_DEPTH}' bundle=gmem2 latency=120/' src/${KERNEL_SRCS}
