...
INPUT_DEPTH=$(( ( (${DIM0}+${TILE_SIZE_DIM0}-3) / (${TILE_SIZE_DIM0}-2) ) * ( (${DIM1}+${TILE_SIZE_DIM1}-3) / (${TILE_SIZE_DIM1}-2) ) * ${TILE_SIZE_DIM0} * ${TILE_SIZE_DIM1} / 32 ))

sed -i 's/#pragma HLS INTERFACE m_axi port=var_sharpen_y offset=slave depth=.* bundle=gmem1 latency=120/#pragma HLS INTERFACE m_axi port=var_sharpen_y offset=slave depth='${INPUT_DEPTH}' bundle=gmem1 latency=120/' src/${KERNEL_SRCS}
